...
function M.new()
	...
	if (prevBasketItems == nil) then
		prevBasketItems = {}
	elseif (#prevBasketItems >= endRange - startRange + 1) then
	...
	-- if (showMoreRandom == 1) then
		...
		-- checkTimeNum = 500
	-- else
	if (tblBasket.numExtraMin ~= nil and tblBasket.numExtraMax ~= nil) then
		if (tblBasket.numExtraMin == -1) then
			if (numRequiredImages > 10) then
				numImages = numRequiredImages
			else
				numImages = numRequiredImages + (10-numRequiredImages)
			end
		else
			numImages = numRequiredImages + math.random(tblBasket.numExtraMin, tblBasket.numExtraMax)
		end
	else
		numImages = numRequiredImages
	end
	...
	function readBasketVO(touched)
		...
		if (currentCount == 1 or touched == true) then
			if savedData.enableSounds == true then
				if (randomVO == 1 or currentGame == 1) then
					playReservedChannel( canYouDragSound, currentGameChannel, 0 )
					dragVOChoice = 1
				else
					playReservedChannel( dragSound, currentGameChannel, 0 )
					dragVOChoice = 2
				end
		...
		local function playItemNumberVO()
			...
			local function playItemNameVO()
				if (currentCount == 1 or touched == true) then
							...
							if savedData.enableSounds == true then
								...
								if (dragVOChoice == 1) then
									if (basketOptions[randomBasket].id == 4) then
										playReservedChannel( toTheHamper2, currentGameChannel, 0 )
									elseif (basketOptions[randomBasket].id == 5) then
										playReservedChannel( toTheToyBox2, currentGameChannel, 0 )
									else
										playReservedChannel( toTheBasket2, currentGameChannel, 0 )
									end
								else
									if (basketOptions[randomBasket].id == 4) then
										playReservedChannel( toTheHamper, currentGameChannel, 0 )
									elseif (basketOptions[randomBasket].id == 5) then
										playReservedChannel( toTheToyBox, currentGameChannel, 0 )
									else
										playReservedChannel( toTheBasket, currentGameChannel, 0 )
									end
								end
					...
					timerNumVO2 = timer.performWithDelay( 750 + basketItemOptions[currentImage].delayVO, toTheBasketVO )
					...
					timerNumVO3 = timer.performWithDelay( 1000, basketWiggle )
	...
	local function moveCurrentItemToBasket( currentItem )
		...
		local function checkCounting()
			...
			if (currentCount > numRequiredImages) then
				...
				timer.performWithDelay( 200, makeButtonsActive ) -- 200
			end
			
		end
		timer.performWithDelay( 330, checkCounting )
    ...
    function this:touch(event)
        ...
        elseif(event.phase == "ended" or event.phase == "cancelled") then
			...
			if (currentItem ~= 0) then
				...
				if (currentX < basketX + (gameBasketBox.contentWidth*0.5) and 
	...
	local function setUpBasket()
		...
		for i = 1, numImages do
			...
			if (#randomColors < 2) then
				gameImages[#gameImages+1] = display.newImageRect("Images/" .. basketItemOptions[currentImage].id .. ".png", basketItemOptions[currentImage].w, basketItemOptions[currentImage].h )
			else
				gameImages[#gameImages+1] = display.newImageRect("Images/" .. basketItemOptions[currentImage].id .. randomColors[randomColor] .. ".png", basketItemOptions[currentImage].w, basketItemOptions[currentImage].h )
			end
