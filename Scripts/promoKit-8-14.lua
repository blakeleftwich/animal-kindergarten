        ...
        elseif ( event.phase == "ended" ) then
         ...
         --   elseif string.find(response,"Page Not Found") > 1 then
          ...
          --       promoKitcompleteFileSynch(false)
				...
				local fileDownload = doesFileExist(candidateSettings[version.appStore].files[fileSynchIndex].fileName)
...
function synchFiles()
	...
	if numFiles == nil then numFiles = 0 end
    printLocal("synchFiles: "..fileSynchIndex.." of "..numFiles .." suffix:"..suffix )
    --if there are more files to get, request them
    if(fileSynchIndex < numFiles + 1) then
        printLocal("synchFiles: checking "..files[fileSynchIndex].fileName.." type:"..files[fileSynchIndex].type.." suffix:"..files[fileSynchIndex].suffix)
		...
		if download then 
			local doesExist = doesFileExist(files[fileSynchIndex].fileName)
			if doesExist then
            ...
            if download then 
                ...
                local fileURL = "http://eggrollgames.com/pk/files/"..fileName
...
local function updatePromoKitSettings() 
	local settingsURL = "http://eggrollgames.com/pk/settings/"..promoKey..".js" --normally use .js
...
function promoKit:init(key,callback,minTime)
    ...
    printLocal("appstore:"..version.appStore)
